...
const Navbar = () => {
    ...
    return (
        <div className='flex justify-evenly items-center w-full h-20 px-4 text-white bg-black opacity-90 fixed gap-40 z-20'>
            <div>
                <Link to='home' smooth duration={500} className='text-5xl font-signature cursor-pointer ml-4'>N<span className='text-red-600'>ancy</span></Link>
            </div>

            <ul className='hidden md:flex'>
                {links.map(({ link, id }) => (
                    <li key={id} className='px-4 cursor-pointer capitalize text-white title-font font-bold font-large hover:scale-125 duration-200 border-b-4 mb-0 md:mb-0 ml-2 text-xl border-transparent hover:border-red-600 transition'>
            ...
            {nav && (
                <ul className='flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 textgray-300'>
                    <h1 className='text-5xl font-signature ml-2 mb-20'>N<span className='text-red-600'>ancy</span></h1>
                    {links.map(({ id, link }) => (
